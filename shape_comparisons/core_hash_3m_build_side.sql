...
-- 3. Hash mode: 3M keys, 10M rows, build-side comparison shape only
-- Run: build/Release/duckdb < shape_comparisons/core_hash_3m_build_side.sql

.print === Hash mode: 3M keys, 10M rows, uniform (build-side comparison shape only) ===
.timer on
