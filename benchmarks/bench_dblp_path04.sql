-- Benchmark: dblp SNAP path04 query from spark-eval
-- Requires: scripts/prepare_dblp_parquet.sh
-- Run:
--   scripts/prepare_dblp_parquet.sh
--   build/Release/duckdb < benchmarks/bench_dblp_path04.sql
--
...
.print === dblp path04, exact spark-eval query on cached parquet ===
...
COPY (
    ...
    WHERE p1.toNode = p2.fromNode
      ...
      AND p3.toNode = p4.fromNode
      AND p4.toNode = p5.fromNode
) TO '/tmp/aggjoin_dblp_path04_1a.csv' (FORMAT CSV);
...
COPY (
    ...
    WHERE p1.toNode = p2.fromNode
      ...
      AND p3.toNode = p4.fromNode
      AND p4.toNode = p5.fromNode
) TO '/tmp/aggjoin_dblp_path04_1b.csv' (FORMAT CSV);
