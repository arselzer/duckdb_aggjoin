...
-- 3. Hash mode: 3M keys, 10M rows, probe-side shape only
-- Run: build/Release/duckdb < shape_comparisons/core_hash_3m_probe_side.sql

.print === Hash mode: 3M keys, 10M rows, uniform (probe-side shape only) ===
.timer on
