-- Benchmark: dblp SNAP path05 query
...
--
-- Exact chain shape:
--   select count(*) from dblp p1, dblp p2, dblp p3, dblp p4, dblp p5, dblp p6
...
.print === dblp path05, exact current-plan query on cached parquet ===
COPY (
    SELECT COUNT(*)
    FROM dblp p1, dblp p2, dblp p3, dblp p4, dblp p5, dblp p6
    WHERE p1.toNode = p2.fromNode
      AND p2.toNode = p3.fromNode
      AND p3.toNode = p4.fromNode
      AND p4.toNode = p5.fromNode
      AND p5.toNode = p6.fromNode
) TO '/tmp/aggjoin_dblp_path05_1a.csv' (FORMAT CSV);
