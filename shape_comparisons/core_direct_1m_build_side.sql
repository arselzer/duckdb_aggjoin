...
-- 2. Direct mode: 1M keys, 10M rows (uniform), build-side comparison shape only
-- Run: build/Release/duckdb < shape_comparisons/core_direct_1m_build_side.sql

.print === Direct mode: 1M keys, 10M rows, uniform (build-side comparison shape only) ===
.timer on
