...
-- 1. Direct mode: 100K keys, 10M rows (uniform), probe-side shape only
-- Run: build/Release/duckdb < shape_comparisons/core_direct_100k_probe_side.sql

.print === Direct mode: 100K keys, 10M rows, uniform (probe-side shape only) ===
.timer on
