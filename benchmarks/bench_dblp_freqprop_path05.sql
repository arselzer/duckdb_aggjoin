-- Benchmark: manual rewrite-y frequency-propagation CTE for dblp path05
-- Uses: benchmarks/data/dblp.parquet
-- Run:
--   build/Release/duckdb < benchmarks/bench_dblp_freqprop_path05.sql
...
.print === dblp freqprop path05, manual rewrite ===
...
WITH
  "_yw_cte_1" AS (
    SELECT "p5"."fromnode", "p5"."tonode",
           SUM(1.0) AS _freq
    FROM "dblp" AS "p5"
    JOIN "dblp" AS "p6" ON "p5"."tonode" = "p6"."fromnode"
  ...
  "_yw_cte_2" AS (
    ...
    JOIN (
      ...
      FROM "_yw_cte_1"
      GROUP BY "fromnode"
    ) AS "_yw_cte_1_agg" ON "p4"."tonode" = "_yw_cte_1_agg"."fromnode"
  ...
  "_yw_cte_3" AS (
    ...
    JOIN (
      ...
      FROM "_yw_cte_2"
      GROUP BY "fromnode"
    ) AS "_yw_cte_2_agg" ON "p3"."tonode" = "_yw_cte_2_agg"."fromnode"
  ...
  "_yw_cte_4" AS (
    ...
    JOIN (
      ...
      FROM "_yw_cte_3"
      GROUP BY "fromnode"
    ) AS "_yw_cte_3_agg" ON "p2"."tonode" = "_yw_cte_3_agg"."fromnode"
  ...
  "_yw_cte_5" AS (
    ...
    JOIN (
      ...
      FROM "_yw_cte_4"
      GROUP BY "fromnode"
    ) AS "_yw_cte_4_agg" ON "p1"."tonode" = "_yw_cte_4_agg"."fromnode"
    GROUP BY "p1"."tonode"
  )
SELECT COALESCE(SUM("_freq"), 0) AS "count_star"
FROM "_yw_cte_5"
) TO '/tmp/aggjoin_dblp_freqprop_path05_enabled.csv' (FORMAT CSV, HEADER);
...
SELECT COALESCE(SUM("_freq"), 0) AS "count_star"
FROM "_yw_cte_5"
) TO '/tmp/aggjoin_dblp_freqprop_path05_disabled.csv' (FORMAT CSV, HEADER);
