...
-- 7. Multi-aggregate: SUM+MIN+MAX+AVG, 1M keys, 10M rows, build-side comparison shape only
-- Run: build/Release/duckdb < shape_comparisons/core_multiagg_1m_build_side.sql

.print === Multi-agg SUM+MIN+MAX+AVG: 1M keys, 10M rows (build-side comparison shape only) ===
.timer on
