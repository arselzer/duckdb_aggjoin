...
-- 5. Sparse keys: 100K rows over a 10M range, probe-side shape only
-- Run: build/Release/duckdb < shape_comparisons/core_sparse_probe_side.sql

.print === Sparse: ~100K keys, range 10M, 100K rows (probe-side shape only) ===
.timer on
