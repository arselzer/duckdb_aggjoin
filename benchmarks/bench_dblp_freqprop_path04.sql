-- Benchmark: manual rewrite-y frequency-propagation CTE for dblp path04
-- Uses: benchmarks/data/dblp.parquet
-- Run:
--   build/Release/duckdb < benchmarks/bench_dblp_freqprop_path04.sql
--
-- Note: this is the pasted rewrite with the broken p1 alias line fixed.
...
.print === dblp freqprop path04, manual rewrite ===
...
WITH
  "_yw_cte_1" AS (
    SELECT "p4"."fromnode", "p4"."tonode",
           SUM(1.0) AS _freq
    FROM "dblp" AS "p4"
    JOIN "dblp" AS "p5" ON "p4"."tonode" = "p5"."fromnode"
  ...
  "_yw_cte_2" AS (
    ...
    JOIN (
      ...
      FROM "_yw_cte_1"
      GROUP BY "fromnode"
    ) AS "_yw_cte_1_agg" ON "p3"."tonode" = "_yw_cte_1_agg"."fromnode"
  ...
  "_yw_cte_3" AS (
    ...
    JOIN (
      ...
      FROM "_yw_cte_2"
      GROUP BY "fromnode"
    ) AS "_yw_cte_2_agg" ON "p2"."tonode" = "_yw_cte_2_agg"."fromnode"
  ...
  "_yw_cte_4" AS (
    ...
    JOIN (
      ...
      FROM "_yw_cte_3"
      GROUP BY "fromnode"
    ) AS "_yw_cte_3_agg" ON "p1"."tonode" = "_yw_cte_3_agg"."fromnode"
    GROUP BY "p1"."tonode"
  )
SELECT COALESCE(SUM("_freq"), 0) AS "count_star"
FROM "_yw_cte_4"
) TO '/tmp/aggjoin_dblp_freqprop_path04_enabled.csv' (FORMAT CSV, HEADER);
...
SELECT COALESCE(SUM("_freq"), 0) AS "count_star"
FROM "_yw_cte_4"
) TO '/tmp/aggjoin_dblp_freqprop_path04_disabled.csv' (FORMAT CSV, HEADER);
