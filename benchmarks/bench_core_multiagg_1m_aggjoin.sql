...
-- 7. Multi-aggregate: SUM+MIN+MAX+AVG, 1M keys, 10M rows, AGGJOIN side only
-- Run: build/Release/duckdb < benchmarks/bench_core_multiagg_1m_aggjoin.sql

.print === Multi-agg SUM+MIN+MAX+AVG: 1M keys, 10M rows (AGGJOIN only) ===
.timer on
