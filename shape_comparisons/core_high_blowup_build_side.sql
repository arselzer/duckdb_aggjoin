...
-- 6. High blowup: 10K keys, 10M rows each (1000x), build-side comparison shape only
-- Run: build/Release/duckdb < shape_comparisons/core_high_blowup_build_side.sql

.print === High blowup: 10K keys, 10M rows (1000x) (build-side comparison shape only) ===
.timer on
